...
import { googleFirebaseKey } from '../../environments/environment';
...
export class AuthService implements OnDestroy {
  ...
  private _user = new BehaviorSubject<User>(null); // user initialized with null;
  ...
  signup(email: string, password: string) {
    return this.http.post<AuthResponseData>(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${googleFirebaseKey}`,
  ...
  login(email: string, password: string) {
    ...
    return this.http.post<AuthResponseData>(`https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${googleFirebaseKey}`,
    // tslint:disable-next-line: object-literal-shorthand
    { email: email, password: password, returnSecureToken: true }
    )
    .pipe(tap(this.setUserData.bind(this)));
